...
module common_cmsui
    ...
    integer,dimension(:,:),allocatable :: ijo_in_h0     !T.K.
...
contains
	...
	subroutine alloc_var1(im, jm)
		...
		allocate( ijobst(0:i,0:j), ijo_in(0:i,0:j), ij_frg(0:i,0:j), ij_ero(0:i,0:j) )
 		allocate( ijo_in_h0(0:i,0:j) )       !T.K.        
...
!$omp do private(i,j)
        ...
        if (time <= 0.0) then      !T.K.
            do j=1,ny                   !T.K.
			    do i=1,nx                     !T.K.
				    if(ijo_in_h0(i,j) == 1) then
                        hs(i,j) = hmin      !T.K.
                        h(i,j) = hs(i,j) + eta(i,j)      !T.K. 
                    endif      !T.K.
			    end do      !T.K.
		    end do      !T.K.
        endif      !T.K.
...
103    continue
       if( j_wl/=1 ) then
         if( h_dse(n) <= emin(nx) ) then
            write(*,*) 'Given Downstream Water Surface is lower than the bed elevation'
            call cg_close_f(fid,ier)
            stop
         end if
       end if
...
!					if( hs(i,j)>hmin .or. hs(i+1,j)>hmin ) then
					if( hs(i,j)>0.005 .or. hs(i+1,j)>0.005 ) then
...
!  CHARACTER (LEN=3), EXTERNAL :: get_extension !rmcd iRIC
  ...
  !ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  !
  integer(4), dimension(:,:), pointer :: obst4, fm4, mix_cell,h0ini_cell        !T.K. h0ini_cell
  ...
  if ( icount.eq.2 ) then
     ...
     allocate (h0ini_cell(ni4-1, nj4-1) )    !T.K.
     ...
     call cg_iric_read_grid_integer_cell_f('Depth0_initial', h0ini_cell, ier)     !T.K.
   ...
   do j=1,ny
      do i=1,nx
       ...
       ! ----- set intial depth0 cells -----

         ijo_in_h0(i,j) = h0ini_cell(i,j)
				! 0: normal cell, 1: h0ini_cell
            ...
            call iric_write_sol_start_f(condFile, ier)   
        	   CALL Write_CGNS(condFile,time,qptemp,im,jm		&
            		,x,y,uxx,uyy,hsxx,z,z0,zb_g,voltex,c_g,dmn,phi_g,fr_g		&
              		, rho, us_g, ts_g,z_ave,z_min,h_ave,qbxx,qbyy,cc_m,nk,j_mix,ij_ero)
...
subroutine write_cgns(InputFile,time,disch,im,jm,x,y,u,v,hs,z		&
						...
						,rho, usta, ts,zave,zmin,have,qbx,qby,cc_m,nk,j_mix,ij_ero)
  use flag_op
  IMPLICIT NONE
  INCLUDE "cgnslib_f.h"
  ...
  INTEGER,dimension(0:im,0:jm),intent(in) ::ij_ero
  ...
  INTEGER, ALLOCATABLE, DIMENSION(:,:) :: IBC, FixedBed
  ...
  ALLOCATE(FixedBed(nx, ny), STAT=ier)
